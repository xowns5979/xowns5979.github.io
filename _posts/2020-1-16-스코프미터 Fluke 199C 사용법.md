...
## Meter 모드
<img src="/assets/scopemeter/meter_screen.png" width="600">
<p style='text-align:center'>Meter 모드 화면 예시</p>
미터 모드 화면은 간단하다. 측정하는 값, 그리고 아래에 막대그래프 정도가 있다. Auto는 자동으로 범위 (0~50kΩ)가 설정되는 모드라는 것.

### Meter모드 키 레이블 메뉴
<img src="/assets/scopemeter/meter_keylabel.png" width="600">
<p style='text-align:center'>Meter 모드 키 레이블</p>
마찬가지로 Meter버튼을 한번 더 누르면 레이블을 숨길 수 있다.
* F1버튼(Measure...): 어떤 수치를 측정할 건지 선택한다 (kΩ, V, A 등)
* F2버튼(Relative On/off): 이 버튼을 누른 순간의 측정값에 대한 상대값 (+, -)을 보여준다.
* F3,F4버튼(Auto / Manual): 디폴트는 Auto다. 자동으로 측정값의 범위를 잡아준다. (숫자 아래 막대 그래프에 범위가 있다). Manual로 선택해서 수동 범위를 지정할수도 있다. F3,F4버튼 뿐만 아니라 청록색 Auto/Manual 버튼으로도 스위치된다.
* 마찬가지로 Hold/Run 버튼으로 화면을 고정할 수 있다.
